...
- 結果画面の概要 + グルメ / スポット / 映え・トレンドの大きな3ボタンとアクセス確認
...
- hero画像内には旅先名・説明文・キャッチコピーなどの文字を入れません。旅先名や説明文はアプリ側UIで表示し、文字入り画像は `rejected` として一般画面には表示しません。
...
結果画面では、旅行先名、適合度、合いそうな短い理由、グルメ/スポット/映え・トレンドの大きな3ボタン、Google Mapsのアクセス確認を中心に短く見せます。詳しい旅程はAIプラン生成に任せ、一般結果画面には運命度カード、モデルコース、交通手段比較を出しません。

### グルメ/スポット/映え・トレンド詳細ページ

- 通常結果画面からは、大きな「グルメ」「スポット」「映え・トレンド」ボタンで別ページ風の詳細表示へ切り替えます。
- グルメ/スポット/映え・トレンド詳細ページでは「DRAW RESULT / 抽選結果」ヘッダーを出さず、旅先名と専用見出しだけで独立した詳細ページとして見せます。
- 詳細ページの上部には「結果に戻る」だけを置き、条件変更、適合度、アクセス確認、保存/比較、再抽選など結果画面用の情報は表示しません。
...
- 映え・トレンドページでは、写真に残したい場所、雰囲気のある立ち寄り候補、映えグルメを `trendHighlights` の手動データで表示します。
- 映え・トレンドは外部SNSや検索から自動取得せず、「最新」「今一番人気」「バズ」などの断定表現を使いません。店名・施設名は `needs_review` として扱い、Google Maps検索リンクで訪問前確認へつなげます。
...
- 結果画面は短い概要を先に見せ、グルメ / スポット / 映え・トレンドの大きな3ボタンで詳細へ進む構成にしています。
- 結果画面の下部にはβテスター向けのフィードバック導線を置き、感想は外部送信せず、コピーまたは端末内保存で共有できるようにしています。名前、メールアドレス、電話番号は入力させません。
...
- カテゴリ画像、汎用画像、ランダム画像は一般結果画面のheroには使いません。画像がない旅先では画像セクションを出さず、グルメ/スポット/映え・トレンドの3ボタンを主役にします。
