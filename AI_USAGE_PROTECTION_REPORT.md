...
- 同じ条件の成功済みプランは10分間、セッションストレージの1件だけのキャッシュから再表示します。
- ブラウザ内の日次目安は5回です。これは**簡易制限**であり、認証や課金制御ではありません。
...
- OpenAI呼び出しは既定28秒で中断し、出力上限は既定1,000 tokensです。`AI_REQUEST_TIMEOUT_MS`は10〜30秒、`AI_MAX_OUTPUT_TOKENS`は400〜1,200の範囲だけ受け付けます。
...
`METHOD_NOT_ALLOWED`、`INVALID_CONTENT_TYPE`、`INVALID_JSON`、`PAYLOAD_TOO_LARGE`、`INVALID_INPUT`、`ORIGIN_NOT_ALLOWED`、`RATE_LIMITED`、`OPENAI_NOT_CONFIGURED`、`UPSTREAM_TIMEOUT`、`UPSTREAM_ERROR`を返します。OpenAIの生エラー、キー、プロンプト全文、スタックトレースは返しません。
