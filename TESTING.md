...
- 結果画面の初期表示では、グルメ / スポット / モデルコース / 移動の深掘りボタンが折り返して表示されることを確認します。
- グルメ、スポット、モデルコース、移動を押したとき、それぞれの詳細だけが表示され、行けそう度・予算目安・薄いおすすめポイントカードが一般結果に出ないことを確認します。
...
- 交通手段比較では、未取得の車ルートや近距離の飛行機など、判断材料にならないカードが大きく表示されないことを確認します。
- ルート確認リンクの目的地が、広すぎる市区町村だけではなく最寄り駅や具体クエリに寄っていることを確認します。
- モデルコースは具体スポット名と具体グルメ名がある場合だけ表示され、データ不足時に汎用文だけで出ないことを確認します。
- 旅のイメージ画像は、旅先専用に近い画像がある場合だけ表示され、カテゴリ fallback だけで横スクロール表示にならないことを確認します。
