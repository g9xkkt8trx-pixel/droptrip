...
- [ ] 検索欄に文字が入っている間は、fallback旅先やおすすめ旅先を混ぜず、検索結果だけを表示する
- [ ] お気に入り登録ができる
- [ ] 比較に追加できる
- [ ] 抽選履歴が保存される
- [ ] スマホ表示が崩れない
...
- [ ] 旅先名が表示される
- [ ] 旅先固定hero画像のうち `confirmed` のものだけ画像が表示される
- [ ] 結果画面heroは `status: confirmed` の画像だけが表示される
- [ ] `needs_review` / `rejected` / `missing` のhero画像は一般画面に表示されない
- [ ] `candidateSrc` は一般画面に表示されない
- [ ] 開発者ページのhero画像レビュー台帳で `src` / `candidateSrc` / `sourceType` / `reviewNote` / `rejectedReason` を確認できる
- [ ] 固定hero画像がない場合は画像枠や「写真準備中」が表示されない
- [ ] 結果画面heroにカテゴリ画像・汎用画像・ランダム画像が代替表示されない
- [ ] 固定hero未設定の旅先で、画像なしでも余白や空枠が残らない
- [ ] 第1弾・第2弾の簡易SVG heroは `rejected` として一般画面に表示されない
- [ ] confirmed未登録の簡易SVG旅先でも、結果画面に空枠や余計な余白が出ない
- [ ] 高品質画像を `confirmed` にするまでは、画像なしの結果画面として自然に成立する
...
- [ ] 鳴門市・別府市・甲府市・川越市のconfirmed heroは各 `hero-v1.webp` を参照し、旧 `hero.svg` を参照していない
...
- [ ] 千葉市・館山市・銚子市・成田市・静岡市・浜松市・伊豆市・沼津市のconfirmed heroは各 `hero-v1.webp` を参照し、旧ローカル候補画像や旧 `hero.svg` を参照していない
- [ ] 富士河口湖町・山中湖村・小田原市・佐野市・新潟市・佐渡市・長野市・小布施町のconfirmed heroは各 `hero-v1.webp` を参照し、旧ローカル候補画像や旧 `hero.svg` を参照していない
- [ ] 三島市・三浦市・逗子市・葉山町・秩父市・川口市・さいたま市・越谷市のconfirmed heroは各 `hero-v1.webp` を参照し、旧ローカル候補画像や旧 `hero.svg` を参照していない
- [ ] 台東区・墨田区・千代田区・新宿区・渋谷区・江東区・浦安市・藤沢市のconfirmed heroは各 `hero-v1.webp` を参照し、旧ローカル候補画像や旧 `hero.svg` を参照していない
...
- [ ] 広島市heroは川沿い・平和記念公園周辺・都市景観・橋など広島らしい要素が伝わる
