...
const getRouteDestinationQuery = (destination = {}) => {
  ...
  const stationQuery = destination.nearestStation
    ? `${destination.nearestStation}, ${destination.prefecture ?? ''}${destination.city ?? ''}`
    : ''
  return destination.routeSearchName
...
const getRouteDestinationLabel = (destination = {}) => (
  ...
  || destination.routeDestinationName
  || destination.routeSearchName
  || destination.googleMapsQuery
  || destination.address
  || `${destination.prefecture ?? ''}${destination.city ?? ''}`
)

const getRouteDestinationSource = (destination = {}) => {
  if (destination.routeSearchName) return 'routeSearchName'
  if (destination.routeDestinationName) return 'routeDestinationName'
  if (destination.nearestStation) return 'nearestStation'
  if (destination.googleMapsQuery) return 'googleMapsQuery'
  if (getConcreteTouristSpots(destination)[0]?.name) return 'touristSpots[0]'
  if (destination.address) return 'address'
...
const firstHeroWaveDestinations = [
  { id: '岐阜県-下呂市', city: '下呂市' },
  { id: '京都府-京都市', city: '京都市' },
  { id: '北海道-小樽市', city: '小樽市' },
  { id: '神奈川県-箱根町', city: '箱根町' },
  { id: '群馬県-草津町', city: '草津町' },
  { id: '静岡県-熱海市', city: '熱海市' },
  { id: '石川県-金沢市', city: '金沢市' },
  { id: '神奈川県-鎌倉市', city: '鎌倉市' },
  { id: '宮城県-仙台市', city: '仙台市' },
  { id: '福岡県-福岡市', city: '福岡市' },
  { id: '徳島県-鳴門市', city: '鳴門市' },
  { id: '大分県-別府市', city: '別府市' },
...
const normalizeSearchText = normalizeDestinationSearchText
...
const getFoodMapsSearchUrl = (destination = {}, food = {}) => {
  const place = destination.prefecture && destination.city
    ? `${destination.prefecture}${destination.city}`
    : destination.city ?? destination.name ?? ''
  const query = [place, food.name].filter(Boolean).join(' ')
...
const localFoodDescriptionRiskPhrases = ['\u98df\u6587\u5316\u306b\u89e6\u308c\u3084\u3059\u3044', '\u5019\u88dc\u306b\u3057\u3084\u3059\u3044', '\u5165\u308c\u3084\u3059\u3044\u3067\u3059', '\u305d\u306e\u571f\u5730\u3089\u3057\u3044\u98df\u4e8b', '\u79fb\u52d5\u306e\u5408\u9593', '\u663c\u98df\u3084\u4f11\u61a9', '\u663c\u98df\u306b\u3059\u308b\u3068']
...
const getFoodDescriptionTravelTimingRiskCities = (destinationList = []) => destinationList
  .filter((place) => (place.localFoodDetails ?? []).some((food) => /タイミング|昼食や休憩|移動の合間|午後の散策|夕食に合わせる|候補にしやすい/.test(String(food?.description ?? ''))))
  .map((place) => place.city)
...
const getAutoGeneratedRestaurantHintCities = (destinationList = []) => destinationList
  .filter((place) => (place.restaurantHints ?? []).some((hint) => /中心部の.+候補|駅周辺の.+候補|観光エリアの.+候補/.test(String(hint?.name ?? ''))))
  .map((place) => place.city)

const getFoodMapLinkMissingCities = (destinationList = []) => destinationList
  .filter((place) => getLocalFoodDetailItems(place, getLocalFoodDisplayItems(place)).some((food) => !place.city || !food.name))
  .map((place) => place.city)
...
function App() {
  ...
  const routeDestinationSource = destination ? getRouteDestinationSource(destination) : ''
      ...
      >
        ...
        {currentPage === 'result' && destination && (
          <div className="result-area" aria-live="polite">
            {resultDetailView === 'food' && (
              <section className="result-detail-page result-food-page" aria-labelledby="result-food-page-title">
                        ...
                        <ul className="local-food-details">
                          {localFoodDetails.map((food) => (
                            <li key={food.name}>
                              ...
                              <a className="food-map-link" href={getFoodMapsSearchUrl(destination, food)} target="_blank" rel="noopener noreferrer">Google Mapsで探す</a>
                            </li>
                          ))}
                        </ul>
                      )}
                ...
                <div className="food-search-guide">
                  ...
                  <p>料理名ごとのボタンから、{destination.prefecture}{destination.city}周辺で検索できます。店舗の営業時間・定休日・提供内容は変わるため、訪問前にGoogle Mapsや公式情報で確認してください。</p>
                </div>
              </section>
            )}
            ...
            <section className="access-confirm-card" aria-labelledby="access-confirm-title">
              <div>
                <p>ACCESS</p>
                <h2 id="access-confirm-title">アクセス確認</h2>
                <span>{planContext.departure} → {routeDestinationLabel}</span>
              </div>
        ...
        <section className="quality-check-card" aria-labelledby="quality-check-title">
          ...
          <details className="image-reuse-details metadata-shortage-details">
            ...
            <dl className="debug-details">
              ...
              <div><dt>食事タイミング寄りの説明文</dt><dd>{formatShortageList(getFoodDescriptionTravelTimingRiskCities(destinations))}</dd></div>
              <div><dt>自動生成restaurantHints残存</dt><dd>{formatShortageList(getAutoGeneratedRestaurantHintCities(destinations))}</dd></div>
              <div><dt>Google Maps検索リンク生成不足</dt><dd>{formatShortageList(getFoodMapLinkMissingCities(destinations))}</dd></div>
        ...
        <section className={`debug-card ${showDebugPanel ? 'expanded' : ''}`} aria-labelledby="debug-title">
          ...
          {showDebugPanel && (
            <dl className="debug-details" id="debug-details">
              ...
              <div><dt>destination list search mode</dt><dd>共通フィールド部分一致</dd></div>
              ...
              <div><dt>general enjoyment chips</dt><dd>{destination ? `${getTripEnjoymentItems(destination, localFoodItems).length}件を一般画面では非表示` : '未確認'}</dd></div>
              ...
              <div><dt>AI送信 restaurantHints</dt><dd>送信なし（料理名と説明文を優先）</dd></div>
