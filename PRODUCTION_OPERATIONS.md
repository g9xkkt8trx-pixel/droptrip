...
- タイトルを連続タップして開く診断画面は、ローカルのDEV環境だけで有効です。ProductionとPreviewでは表示・遷移ともに無効化されています。
- 連続タップやlocalStorageの値は認証手段ではありません。管理操作をクライアント側の隠し導線へ依存させず、Productionの運用操作はVercelと各外部サービスの管理画面で行います。

## 問い合わせフォーム

- 一般ユーザー向けの問い合わせ先は`VITE_CONTACT_FORM_URL`で設定します。Googleフォームなどの`https://` URLをProductionへ登録し、必要ならPreviewにも登録します。Sensitive指定は不要です。
- 未設定または不正なURLでは、`/contact.html`は「お問い合わせフォームは準備中です」と表示し、外部リンクを出しません。
- フォームURLはお問い合わせページだけで利用し、フッターや法務ページへ直接掲載しません。一般配信UIに開発者個人やソース管理サービスへの導線を置きません。
