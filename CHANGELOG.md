...
- タイトルの連続タップで開く開発者向け診断画面をDEV環境限定にしました。ProductionとPreviewでは入口、画面遷移、表示分岐、診断用遅延読み込みを無効化し、隠し操作を認証手段として扱わない構成にしています。
- 一般ユーザー向け問い合わせ導線を外部フォーム設定へ移行しました。公開UI・配布物からGitHubへの問い合わせ導線を除去し、`VITE_CONTACT_FORM_URL`が未設定または不正な場合は外部送信ボタンを表示しません。
- Web版の機能を維持したまま、manifest、既存faviconを踏襲したPWAアイコン、ネットワーク優先のService Worker、オフライン案内、更新通知、対応ブラウザ向けインストール導線を追加しました。AI旅行プラン、移動時間、`/api/`はService Workerでキャッシュしません。
- 正式公開向けに、利用規約・プライバシーポリシー・問い合わせ導線・共通フッター・サービス注意表示を追加しました。
- AI旅行プランと移動時間取得を、サーバー環境変数で個別にメンテナンス停止できるようにしました。
- `PRODUCTION_OPERATIONS.md` と `PRODUCTION_SMOKE_TEST.md` を追加し、デプロイ、ロールバック、ログ、利用量、障害対応、定期確認を整理しました。
...
- 映えスポット対応の最終監査を行い、未登録だった松島町など13旅先へ公式情報を確認した39件をconfirmed登録しました。正式旅先137件を重複除外した136自治体すべてで3件ずつ、合計408件の映えスポットを表示可能にしています。検証スクリプトは各正式旅先のconfirmed 3件以上、情報源、重複、存在しないdestinationIdを継続確認します。初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のまま、遅延`photoSpots`チャンクは156.40KB（gzip 35.21KB）です。
- 映えスポット実データ第10弾として、秋田市など15旅先へ45件をconfirmed登録しました。5旅先ずつ3工程で検証し、映えスポットは全369件、登録旅先は123件になりました。正式旅先の未登録13件と次回候補は監査台帳へ記録しています。初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のまま、遅延チャンクは141.65KB（gzip 31.72KB）です。
- 映えスポット実データ第9弾として、町田市など15旅先へ45件をconfirmed登録しました。5旅先ずつ3工程で検証し、映えスポットは全324件となりました。初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のまま、遅延チャンクは126.74KB（gzip 28.44KB）です。
- 映えスポット実データ第8弾として、墨田区など15旅先へ45件をconfirmed登録しました。5旅先ずつ3工程で重複を確認し、映えスポットは全279件となりました。初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のまま、遅延チャンクは112.20KB（gzip 25.41KB）です。
- 映えスポット実データ第7弾として、名古屋市など15旅先へ45件をconfirmed登録しました。第1工程5旅先と残り10旅先に分け、既存データを変更せず重複を回避しています。映えスポットは全234件となり、初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のまま、遅延チャンクは94.80KB（gzip 21.65KB）です。
- 映えスポット実データ第6弾として15旅先・45件をconfirmed登録しました。中断後は未完成旅先だけを分割継続して重複を回避し、映えスポットは全189件となりました。初期エントリーJavaScriptは435.18KBのまま、遅延チャンクは76.25KB（gzip 17.35KB）です。
- 映えスポット実データ第5弾として、那須塩原市など15旅先へ45件をconfirmed登録しました。映えスポットは全144件となり、初期エントリーJavaScriptは435.18KBのまま、遅延チャンクは59.98KB（gzip 13.97KB）です。
- 映えスポット実データ第4弾として、奈良市・鎌倉市・金沢市・神戸市・広島市・松山市・高山市・長崎市・別府市・仙台市・日光市・伊勢市・倉敷市・高知市・鹿児島市へ、公式情報を確認した45件をconfirmed登録しました。映えスポットは全99件となり、初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のまま、遅延チャンクは43.94KB（gzip 10.45KB）です。
- 映えスポット実データ第3弾として、横浜市・札幌市・釧路市・天草市・白浜町・富山市・佐世保市・山口市・明石市・鳥羽市へ、公式情報を確認した30件をconfirmed登録しました。映えスポットは全54件となり、初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のまま、遅延チャンクは27.71KB（gzip 6.69KB）です。
- 映えスポット実データ第2弾として、箱根町・京都市・小樽市・函館市へ公式観光サイトで確認した12件をconfirmed登録しました。合計24件はすべて詳細画面の遅延チャンクで読み込み、初期エントリーJavaScriptは435.18KB（gzip 120.69KB）のままです。
- 映えスポットの実データ第1弾として、那覇市・石垣市・宮古島市・本部町へ公式または施設公式サイトで確認した12件をconfirmed登録しました。情報源名・URL・確認日を監査用に保存し、confirmedだけを遅延読み込み後の「映え」詳細へ表示します。初期エントリーJavaScriptは435.18KB（gzip 120.69KB）で据え置き、`photoSpots`遅延チャンクは7.14KB（gzip 2.13KB）です。
- 映えスポットの現状監査を行い、confirmedのみを一般表示する専用データ基盤と検証スクリプトを追加しました。既存`trendHighlights`は全件needs_reviewのため移行せず、映え詳細を開いた時だけ専用データを読み込む構成にしています。
- 初期描画後に旅先データを読み込む構成へ変更し、開発者ページ専用の品質・抽選バランス診断も遅延読み込みにしました。エントリーJavaScriptは 805.85KB（gzip 196.07KB）から 433.34KB（gzip 120.34KB）になり、旅先検索・hero表示の仕様は維持しています。
- 旅先一覧検索を正式旅先全件の共通検索処理へ統一し、新規旅先が検索結果から漏れる問題を修正しました。都道府県・市区町村・都道府県＋市区町村・既存タグ等を同じ正規化で検索し、表示時は既知の豊岡市重複を1件へ安全に整理します。全137件を検索語として検証するスクリプトも追加しました。
...
- 登別市・平泉町・足利市・上山市・酒田市・花巻市・八戸市・郡山市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録しました。
- 高島市・下関市・萩市・三好市（徳島県）・佐賀市・嬉野市・北九州市・釧路市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録しました。
- 那須塩原市・佐世保市・仙北市・湯沢町・富山市・立山町・坂井市・鳥羽市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録しました。館山市と立山町の `tateyama` パス衝突は、館山市をHEAD版へ復元し、立山町を `tateyama-toyama/hero-v1.webp` へ分離して解消しました。
...
- 三島市・三浦市・逗子市・葉山町・秩父市・川口市・さいたま市・越谷市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録しました。
- 台東区・墨田区・千代田区・新宿区・渋谷区・江東区・浦安市・藤沢市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録しました。
...
- 松山市（道後温泉）・豊岡市（城崎温泉）・尾花沢市（銀山温泉）の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧 `hero.svg` は一般画面で使わないようにしました。
- 奈良市・日光市・伊勢市・横浜市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。
- 宇都宮市・会津若松市・白川村・富良野市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。宇都宮市は現行の抽選用旅先データ未登録のため、画像管理側で先行登録しています。
- 那須町・奥多摩町・指宿市・鹿児島市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。
- 軽井沢町・上田市・松本市・諏訪市・水戸市・大洗町・日立市・土浦市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。諏訪市・日立市・土浦市は現行の抽選用旅先データ未登録のため、画像管理側で先行登録しています。
- 鳥取市・出雲市・高松市・高知市・熊本市・阿蘇市・宮崎市・日南市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。出雲市・阿蘇市・日南市は現行の抽選用旅先データ未登録のため、画像管理側で先行登録しています。
- 青森市・弘前市・盛岡市・秋田市・山形市・米沢市・福島市・渋川市（伊香保温泉）の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。米沢市・福島市・渋川市は現行の抽選用旅先データ未登録のため、画像管理側で先行登録しています。
- 大阪市・神戸市・姫路市・和歌山市・岐阜市・名古屋市・福井市・敦賀市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。神戸市は有馬温泉寄りの画像から港町heroへ差し替え、姫路市・岐阜市は現行の抽選用旅先データ未登録のため画像管理側で先行登録しています。
- 千葉市・館山市・銚子市・成田市・静岡市・浜松市・伊豆市・沼津市の文字なし `hero-v1.webp` を高品質AI生成hero画像として `confirmed` 登録し、旧ローカル候補画像や旧 `hero.svg` は一般画面で使わないようにしました。千葉市・銚子市・成田市・沼津市は現行の抽選用旅先データ未登録のため、画像管理側で先行登録しています。
