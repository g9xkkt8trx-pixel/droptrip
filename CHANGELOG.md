...
- 追加したSVGは `destination_fixed` / `needs_review` / `isIllustration: true` として登録し、現地写真と誤認させないaltで管理します。
...
- `IMAGE_TODO.md` を全119旅先の固定hero画像管理表へ更新し、固定heroあり30件・未作成89件、優先旅先の整備状況、推奨ファイルパス、画像サイズ/形式ルールを追跡できるようにしました。
