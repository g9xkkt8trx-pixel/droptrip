...
- 複数工程のバッチは、完了済み旅先を変更せず、最終工程で全対象旅先が必要件数に達していることを再集計します。
- 大規模バッチは5旅先程度の工程単位でID・名称・旅先キー・情報源URLを確認してから次工程へ進みます。
- 各工程後と最終工程後に、全対象のconfirmed件数・重複・存在しないdestinationIdを検証します。
- 登録対象を使い切った段階では、正式旅先と`destinationId`を照合し、未登録旅先と次回候補を監査台帳へ残します。
- 最終監査では、`prefecture-city`キーで重複を除いた正式旅先の全件でconfirmedスポットが3件以上あることを検証します。既知の旅先データ重複は元データを変更せず、監査側で1自治体として扱います。
